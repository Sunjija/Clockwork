...
- 런타임 에셋 리비전을 8로 올리고 `generated/v3/`만 참조하도록 변경했다.
...
- 신규 생성 후보 중 비율이 흔들린 대기·점프·더블점프·공격 이미지는 채택하지 않았다. 해당 클립은 승인 프레임을 v3 구조로 옮긴 상태다.
- 걷기와 대시는 실제 플레이에서 추가 속도 튜닝이 필요할 수 있다. 프레임을 바꾸더라도 640×512 캔버스와 `(320, 480)` 소스 피벗은 유지한다.
- 피격, 다운, 기상, 위 공격, 아래 공격 전용 다중 프레임은 아직 제작하지 않았다.
- 다음 작업은 공격 다리 길이와 점프 동작의 체감 비율을 Unity 기준 캐릭터 실루엣으로 재검수하는 것이다.
